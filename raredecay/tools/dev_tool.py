...
def make_logger(
    ...
    if logging_mode == "both" or logging_mode == "file":
        ...
        fh = logging.FileHandler(
            f"{log_file_fullname}-{time_stamp}-logfile.txt", file_mode
        )
...
def add_file_handler(
    ...
    fh = logging.FileHandler(f"{log_file_fullname}-{time_stamp}-logfile.txt", file_mode)
...
def is_in_primitive(test_object, allowed_primitives=None):
    ...
    if isinstance(test_object, (list, np.ndarray, pd.Series, pd.DataFrame)):
        flag = False
    elif isinstance(allowed_primitives, collections.abc.Iterable) and (
...
def play_sound(duration=0.3, frequency=440):
    ...
    os.system(
        f"play --no-show-progress --null --channels 1 synth {duration} sine {freq:f}"
    )
